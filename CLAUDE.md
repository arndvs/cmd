...
This folder uses a two-file memory system:

- **`CLAUDE.md`** (this file) — explicit instructions, routing rules, "how the operator wants things done." Authored by you. Brief, opinionated, high-signal. **Not a knowledge dump.**
- **`MEMORY.md`** (next to this file) — auto-memory, maintained by the AI. Captures patterns and preferences learned from working with you.

**Critical:** Routing rules belong here in `CLAUDE.md`. Observed learnings belong in `MEMORY.md`. The folder structure below holds the bulk of context — these two files are the operating manual on top.
...
**Secrets never live inside this repo.** The `.gitignore` blocks common patterns but those are safety nets, not the contract.

The contract: secrets live in a dedicated directory outside the repo with owner-only permissions. Load via shell substitution at the moment of use. Never `cat`, `echo`, or paste secrets into a transcript, log, or AI conversation.

---

## Rules — non-negotiable

1. **Operator takes responsibility.** AI extends, never replaces, the operator's judgment.
2. **Secrets are sacred.** Never read, log, or commit credentials, tokens, or keys.
3. **No new files at root.** The root set is fixed.
4. **No new top-level directories** without explicit operator approval.
5. **Append-only for logs and decisions.** Never rewrite history.
6. **AI quality = context quality.** If the output is junior, the context is junior. Improve the folder before blaming the model.
7. **Markdown first.** No proprietary formats for knowledge.
...
- ctrl+shft routes → cmd stores
- ctrl+shft detects which venture you're working in → loads that venture's cmd context
- ctrl+shft skills handle code → cmd skills handle business operations
- The `cmd-venture:` field in ctrl+shft's `client.instructions.md` bridges the two systems
