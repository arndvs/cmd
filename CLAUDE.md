...
The `cadence/` directory holds operational rhythms. Templates live in `cadence/_template/`, completed reviews in `cadence/reviews/`.

- `_template/daily.md` — morning scan + build block + EOD ritual
- `_template/weekly.md` — 5 review questions, content shipped, carryovers
- `_template/monthly.md` — strategic alignment check, pattern spotting
- `_template/quarterly.md` — OKR setting, rock definition, kill-list
- `_template/yearly.md` — full retrospective, theme setting, big bets
...
4. **No credentials or secrets in this repo.** Secrets live outside `cmd` and are provided via `ctrl+shft`, environment variables, or an external secrets manager.
5. **Markdown first.** No proprietary formats for knowledge.
